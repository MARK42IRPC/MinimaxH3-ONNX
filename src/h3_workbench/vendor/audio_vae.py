...
class Snake1d(nn.Module):
    ...
    def forward(self, x):
        alpha = torch_compat.cast_to_input(self.alpha, x)
...
class SnakeBeta(nn.Module):
    ...
    def forward(self, x):
        alpha = torch.exp(torch_compat.cast_to_input(self.alpha, x)).view(1, -1, 1)
        beta = torch.exp(torch_compat.cast_to_input(self.beta, x)).view(1, -1, 1)
...
class UpSample1d(nn.Module):
    ...
    def forward(self, x):
        C = self.channels if self.channels is not None else x.shape[1]
        x = F.pad(x, (self.pad, self.pad), mode="replicate")
        x = F.conv_transpose1d(x, torch_compat.cast_to_input(self.filter.expand(C, -1, -1), x), stride=self.stride, groups=C).mul_(self.ratio)
...
class LowPassFilter1d(nn.Module):
    ...
    def forward(self, x):
        ...
        return F.conv1d(x, torch_compat.cast_to_input(self.filter.expand(C, -1, -1), x), stride=self.stride, groups=C)
...
class Activation1d(nn.Module):
    ...
    def __init__(self, activation, up_ratio=2, down_ratio=2, up_kernel_size=12, down_kernel_size=12):
        ...
        channels = int(activation.alpha.numel())
        self.upsample.channels = channels
        self.downsample.lowpass.channels = channels
...
class CausalAttention(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        weight, _, offload_stream = torch_compat.cast_bias_weight(self.qkv, x, offloadable=True)
        qkv = F.linear(x, weight=weight, bias=torch_compat.cast_to_input(torch.cat((self.q_bias, self.zero_k_bias, self.v_bias)), x))
        torch_compat.uncast_bias_weight(self.qkv, weight, None, offload_stream)
        ...
        x = torch_compat.scaled_dot_product_attention(q, k, v, is_causal=True)
