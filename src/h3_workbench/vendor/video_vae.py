...
from h3_workbench import torch_compat

optimized_attention = torch_compat.optimized_attention

ops = torch_compat.disable_weight_init
...
class Attention(nn.Module):
    ...
    def forward(self, x, rotary_pos_emb=None):
        ...
        query = torch_compat.rms_norm(query, self.norm_q.weight, self.norm_q.eps)
        key = torch_compat.rms_norm(key, self.norm_k.weight, self.norm_k.eps)

        if rotary_pos_emb is not None:
            rot = rotary_pos_emb.shape[-3] * 2
            rotated_query, rotated_key = torch_compat.apply_rope_split_half(
                query[..., :rot], key[..., :rot], rotary_pos_emb)
            query = torch.cat((rotated_query, query[..., rot:]), dim=-1)
            key = torch.cat((rotated_key, key[..., rot:]), dim=-1)
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, rotary_pos_emb=None):
        attention = self.attn(torch_compat.rms_norm(x, self.norm1.weight, self.norm1.eps), rotary_pos_emb)
        x = x + attention * torch_compat.cast_to_input(self.scale1, x)
        feed_forward = self.ff(torch_compat.rms_norm(x, self.norm2.weight, self.norm2.eps))
        return x + feed_forward * torch_compat.cast_to_input(self.scale2, x)
...
class ViT3DDecoder(nn.Module):
    ...
    def _offloaded_block(self, block, x, rotary_pos_emb, device):
        with torch.device("meta"):
            device_block = TransformerBlock(**self.block_config)
        state = {name: value.to(device) for name, value in block.state_dict().items()}
        device_block.load_state_dict(state, strict=True, assign=True)
        output = device_block(x, rotary_pos_emb)
        del device_block, state
    ...
    def forward(self, x):
        ...
        offload_device = torch.device(self.offload_device) if self.offload_device is not None else None
        if offload_device is not None:
            x = x.to(offload_device)
            self.x_embedder.to(offload_device)

        h = self.x_embedder(x.flatten(2).transpose(1, 2))  # [B, T*H*W, C]
        if offload_device is not None:
            self.x_embedder.to("cpu")

        num_patches = h.shape[1]
        num_suffix = 1 + self.num_register_tokens

        h = torch.cat([h, torch_compat.cast_to_input(self.register_tokens, h).expand(B, -1, -1), torch.zeros_like(h[:, 0:1, :])], dim=1)
        ...
        for block in self.transformer_blocks:
            if offload_device is None:
                ...
                h = self._offloaded_block(block, h, rotary_pos_emb, offload_device)

        if offload_device is not None:
            self.norm_out.to(offload_device)
            self.proj_out.to(offload_device)
...
class MiniMaxH3VideoVAE(nn.Module):
    ...
    def _decode_pixels(self, z):
        offload_device = self.decoder.offload_device
        if offload_device is None:
            return self.decoder(self.post_quant_conv(z))
        device = torch.device(offload_device)
        self.post_quant_conv.to(device)
        hidden = self.post_quant_conv(z.to(device))
        self.post_quant_conv.to("cpu")
        return self.decoder(hidden).to(z.device)
    ...
    def _decode_temporal_chunks(self, z_len):
        ...
        if num_chunks < 1:
            # Too few tokens for one window (for example T_lat == 2).
    ...
    def decode_temporal(self, z, output_buffer=None):
        ...
        if output_buffer is None:
            ...
            output_buffer = torch.empty(self.decode_output_shape(z.shape), dtype=torch.float32,
                                        device=torch_compat.intermediate_device())
