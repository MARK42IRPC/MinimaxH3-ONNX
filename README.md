...
- Super-resolution inputs must stay inside the workspace or be uploaded through the WebUI. Output is limited to 2048 pixels per side and 4 MP. Decoded RGB input frames use an in-memory budget of 2 GiB, while segmented processing uses 22-frame Video VAE windows with a 17-frame stride. The encoder requires the validated `progressive_staged_cuda_v1` ONNX product; automatic PyTorch fallback is disabled. On low-VRAM CUDA devices, fixed-shape stage 0/1 tiles use cached TensorRT FP16 engines and compact tail/head stages use CUDA EP. Set `H3_VAE_TENSORRT=0` for a CUDA-only comparison or `H3_VAE_TENSORRT_CACHE` to move the engine cache. CUDA OOM retries smaller progressive tiles.
...
- Segmented mode uses independent edge-safe clips and can jump at internal boundaries because automatic cross-segment continuation is not wired. Explicit first/last references condition the outermost segments only.
...
- Main-model shards use a three-level pipeline: ONNX files on SSD, bounded asynchronous read-ahead in system RAM, and persistent topology execution in VRAM. The persistent path keeps an eight-graph rolling window by default. Two ordered SSD workers warm source files first, then three weight workers copy from the page cache into reusable host buffers, so graph consumption does not initiate physical I/O. `H3_WEIGHT_PREFETCH_DEPTH` (0-12), `H3_WEIGHT_PREFETCH_WORKERS` (1-4), and `H3_L2_PREFETCH_WORKERS` (1-4) override these defaults.
- `H3_L2_CACHE_GIB` overrides the general RAM read-ahead budget (default: available RAM minus an 8 GiB reserve, clamped to 1-12 GiB). When the persistent hot set is active its automatic L2 window is capped at 3 GiB by default; `H3_WEIGHT_L2_CACHE_GIB` changes that cap. `H3_PREFETCH_SHARDS` controls the non-persistent look-ahead depth (default: 16 graphs). Set `H3_L2_CACHE_GIB=0` to disable read-ahead for comparison.
- CUDA jobs also build a task-level host-RAM weight hot set before denoising. Auto mode caches as many persistent QKV, Attention output, and MLP weights as fit after a 12 GiB safety reserve plus a 2 GiB L2 reserve; all task segments reuse that set. Set `H3_WEIGHT_RAM_CACHE=0` to disable it, or use `H3_WEIGHT_RAM_CACHE_GIB` to cap it. `H3_WEIGHT_RAM_WORKERS` controls background loading. The rolling queue has its own 8 GiB cap and preserves 4 GiB of currently available host memory; tune these with `H3_WEIGHT_PREFETCH_RAM_GIB` and `H3_WEIGHT_PREFETCH_RESERVE_GIB`. The complete 37.93 GiB main product only becomes fully resident on hosts with enough free RAM; a 32 GiB host receives a bounded hot set and streams the remainder.
